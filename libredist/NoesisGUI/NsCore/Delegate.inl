...
private:
    void FromFreeFunc(Ret (*f)(Args...))
    {
        ...
        new(Placement(), data) FreeFuncStub<F>(f);
    }
    ...
    void FromFunctor(const F& f, Ret(F::*)(Args...))
    {
        ...
        new(Placement(), data) FunctorStub<F>(f);
    }
    ...
    void FromMemberFunc(C* obj, Ret (C::*f)(Args...))
    {
        ...
        new(Placement(), data) MemberFuncStub<C, F>(obj, f);
    }
    ...
    void FromMemberFunc(const C* obj, Ret (C::*f)(Args...) const)
    {
        ...
        static_assert(sizeof(MemberFuncStub<C, F>) <= sizeof(data), "Insufficient buffer size");
        new(Placement(), data) MemberFuncStub<C, F>(const_cast<C*>(obj), f);
    }

    void Init()
    {
        static_assert(sizeof(NullStub) <= sizeof(data), "Insufficient buffer size");
        new(Placement(), data) NullStub();
    }
    ...
    public:
        ...
        {
            new(Placement(), dest) NullStub(*this);
        }
    ...
    protected:
        ...
        void Add(const Delegate& d) override
        {
            if (!d.Empty())
            {
                ...
                MultiDelegate* multiDelegate = new(Placement(), this) MultiDelegate();
    ...
    public:
        ...
        uint32_t Size() const override
        {
            return mVector->v.Size() - mVector->compactPending;
        }
        ...
        struct InvokerGuard
        {
            ...
            ~InvokerGuard()
            {
                if (--_v->nestingCount == 0 && _v->compactPending > 0)
                {
                    _v->Compact();
                }
            }
        ...
        void Remove(const Delegate& d) override
        {
            if (!d.Empty())
            {
                ...
                {
                    it->Reset();
                    mVector->compactPending++;
                }

                if (mVector->nestingCount == 0 && mVector->compactPending > 0)
                {
                    mVector->Compact();
                }
            }

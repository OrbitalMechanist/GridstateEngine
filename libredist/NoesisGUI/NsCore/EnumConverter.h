...
private:
    virtual Ptr<BaseComponent> Box(uint64_t v) const = 0;
    virtual uint64_t Unbox(BaseComponent* value) const = 0;
...
private:
    Ptr<BaseComponent> Box(uint64_t v) const override;
    uint64_t Unbox(BaseComponent* value) const override;
